...
function GM:ScoreboardShow()
	...
	if IsValid(scoreboard) then
		...
		for k, v in pairs( player.GetAll() ) do
			if ( teams[v:Team()].base == 1 and v:Team() != 1 ) then
			...
			if ( teams[v:Team()].base == 2 and v:Team() != 2 ) then
			...
			if ( v:Team() == TEAM_SPECTATOR or v:Team() == TEAM_UNASSIGNED or v:Team() == 0 or v:Team() == 1 or v:Team() == 2 ) then
				...
				t3_pPanel.Paint = function()
					draw.RoundedBox( 0, 0, 0, t3_pPanel:GetWide(), t3_pPanel:GetTall(), teams[v:Team()].color )
